...
func sixteen(nums []int, start, end int) []int {
	pivot := start
	temp := nums[pivot]
	i, j := start, end
	for i <= j {
...
func eighteen(nums []int, start, end int) []int {
	pivot := start
	temp := nums[pivot]
	i, j := start, end
	for i <= j {
	...
	nums[pivot] = temp
	if pivot-start > 1 {
		eighteen(nums, start, pivot-1)
	}
	if pivot+1 < end {
		eighteen(nums, pivot+1, end)
	}
	return nums
}
