...
func thirteen(interval [][]int) int {
	sort.Slice(interval, func(i, j int) bool {
		return interval[i][0] < interval[i][1]
	})
	var end []int
	for i := 0; i < len(interval); i++ {
		if len(end) == 0 {
...
func fourteen(interval [][]int) int {
	sort.Slice(interval, func(i, j int) bool {
		return interval[i][0] < interval[j][0]
	})
	var end []int
